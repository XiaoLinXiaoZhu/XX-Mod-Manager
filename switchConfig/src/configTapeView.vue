...
function initLight(scene) {
  ...
  const _helper = new THREE.DirectionalLightHelper(light, 5);
  // scene.add(helper);
  ...
  setTimeout(() => {
    const bulbTween = new Tween({ intensity: 0 })
      .to({ intensity: 20 }, 10)
      .yoyo(true)
      .delay(90)
      .repeat(6)
...
async function initTapes(scene) {
  ...
  while (tapeCount < 15) {
    configs.forEach((config) => {
      ...
      // debug
      console.log(
        `[${new Date().toLocaleTimeString()}] create tape ${tapeCount} with title: ${config.title}`,
      );
      const _tapeInstance = tape.GetTape();
    });
  }
...
function initPostProcessing(scene, camera, renderer) {
  ...
  const _dof2 = new BokehPass(scene, camera, {
    focus: 30,
    aperture: 1000,
    maxblur: 0.01,
  });
...
function showLogo() {
  ...
  // logo 从远端旋转飞到屏幕中央
  const logoTween = new Tween({ z: 21 })
    .to({ z: 16 }, 2000)
  ...
  const logoRotateTween = new Tween({ x: 0, y: -3, z: 0 })
    .to({ x: 0, y: 0, z: 0 }, 2000)
    .easing(Easing.Quadratic.Out)
    .onUpdate((object) => {
...
function onClick() {
  ...
  console.log(
    `[${new Date().toLocaleTimeString()}] click tape ${currentIndex}`,
  );
  tapeManager.onClick();
}
...
function nextTape() {
  console.log(
    `click next, currentIndex: ${tapeManager.currentIndex} of ${tapeManager.tapes.length}`,
  );
...
function prevTape() {
  console.log(
    `click prev, currentIndex: ${tapeManager.currentIndex} of ${tapeManager.tapes.length}`,
  );
  tapeManager.RollTape(-1);
  refreshTapeInfo();
}
...
<svg height="100%" width="100%" viewBox="0 0 512 512" xmlns="http://www.w3.org/2000/svg" xml:space="preserve" version="1.1" fill="#000000">
    ...
    btn.addEventListener("mouseover", (_e) => {
      const svg = btn.querySelector("g");
      svg.style.fill = "gray";
    });

    btn.addEventListener("mouseout", (_e) => {
      const svg = btn.querySelector("g");
    ...
    btn.addEventListener("mouseup", (_e) => {
      const svg = btn.querySelector("g");
      //闪烁一下荧光绿
