...
class Tape {
	...
	//--------------texture----------------
	static defaultfrontTexture = textureLoader.load(
		`./src/assets/tape_front.jpg`,
	);
	...
	static defaultspineTexture = textureLoader.load(
		`./src/assets/tape_spine.png`,
	);
	...
	SetTapeMaterialFromUrl(urls) {
		...
		const frontTexture = fs.existsSync(urls.tape_front)
			? textureLoader.load(createObjectURLFromLocalFile(urls.tape_front))
			: Tape.defaultfrontTexture;
		const sideTexture = fs.existsSync(urls.tape_side)
			? textureLoader.load(createObjectURLFromLocalFile(urls.tape_side))
			: Tape.defaultsideTexture;
		const backTexture = fs.existsSync(urls.tape_back)
			? textureLoader.load(createObjectURLFromLocalFile(urls.tape_back))
			: Tape.defaultbackTexture;
		const spineTexture = fs.existsSync(urls.tape_spine)
			? textureLoader.load(createObjectURLFromLocalFile(urls.tape_spine))
			: Tape.defaultspineTexture;
	...
	GetTape() {
		if (this.instance === null) {
			// 创建磁带盒
			const materials = createTapeMaterial(
				this.frontTexture,
				this.sideTexture,
				this.backTexture,
				this.spineTexture,
	...
	RotationByAxis(axis, angle, originRotation = { x: 0, y: 0, z: 0 }) {
		...
		//debug
		console.log(
			`[${new Date().toLocaleTimeString()}] Rotation by axis ${axis.x}, ${axis.y}, ${axis.z} and angle ${angle} to euler ${euler.x}, ${euler.y}, ${euler.z}`,
		);
		const targetRotation = {
			x: originRotation.x + euler.x,
			y: originRotation.y + euler.y,
			z: originRotation.z + euler.z,
		};
	 ...
	 */
	calculateEulerAnglesFromInitialToTarget(
		targetNormal,
		rotationAngle,
		...
		const _axis = new THREE.Vector3()
			.crossVectors(initialNormal, targetNormal)
			.normalize();

		// 计算旋转角度：初始法线到目标法线的夹角
		const _angle = initialNormal.angleTo(targetNormal);

		// 创建旋转四元数，表示从初始法线到目标法线的旋转
		const quaternionToTarget = new THREE.Quaternion().setFromUnitVectors(
			initialNormal,
			targetNormal,
		);

		// 创建旋转四元数，表示绕目标法线旋转指定的角度
		const quaternionRotation = new THREE.Quaternion().setFromAxisAngle(
			targetNormal,
			rotationAngle,
		);
...
class TapeManager {
	...
	RollTape(dertaIdx) {
		this.currentIndex =
			(this.currentIndex + dertaIdx + this.tapes.length) % this.tapes.length;
		//debug
		...
		for (let i = 0; i < length; i++) {
			...
			this.tapes[i].targetRotation = this.getRotationByVirtualIndex(
				virtualIndex,
				this.tapes.length,
			);
			...
			if (length % 2 === 0) {
				if (dertaIdx < 0 && virtualIndex === -halfLength + 1) {
					...
					const position = this.getPositionByVirtualIndex(
						-halfLength - 2,
						this.tapes.length,
					);
	...
	getPositionByVirtualIndex(virtualIndex, _length) {
	...
	getRotationByVirtualIndex(virtualIndex, _length) {
