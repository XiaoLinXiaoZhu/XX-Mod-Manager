...
class IManager {
  ...
  async loadMods() {
    ...
    this.data.modList = await Promise.all(
      loadMods.map(async (mod) =>
        ModData.fromJson(mod).setModSourcePath(modSourcePath),
      ),
    );
    ...
    console.log(
      `成功加载 ${loadMods.length} 个 mod，总共 ${this.data.characterList.length} 个 角色`,
    );
    return loadMods;
  }
  ...
  async newLoadMods() {
    ...
    const _loadModData = await ModLoader.loadMods();
  }
  ...
  async afterModsLoad() {
    ...
    //debug
    console.log(
      `成功加载 ${loadModData.length} 个 mod，总共 ${this.data.characterList.length} 个 角色,有 ${this.newMods.length} 个新mod`,
    );
    return loadRawMods;
  }
  ...
  async refreshCharacterList() {
    ...
    if (
      this.temp.currentCharacter !== null &&
      !this.data.characterList.includes(this.temp.currentCharacter)
    ) {
  ...
  async loadModInfo(modName) {
    const { modInfo, modData } = await ModLoader.loadMod(
      path.join(this.config.modSourcePath, modName),
    );
  ...
  async loadModInfoOld(modName) {
    ...
    // 将其转换为 ModData
    const mod = ModData.fromJson(data).setModSourcePath(
      this.config.modSourcePath,
    );
  ...
  async extractArchive(archivePath, destinationPath) {
    ...
    while (callbackCount > 0 || !startCallback) {
      ...
      if (Date.now() - startTime > maxWaitTime && !startCallback) {
        console.error(
          `Error: Extract can't start.The archive may be damaged or is empty`,
        );
        t_snack(
          {
  ...
  async handleArchiveDrop(file) {
    ...
    if (ifSuccess) {
      ...
      //debug
      console.log(
        `currentCharacter: ${this.temp.currentCharacter}`,
        mod.character,
      );
      if (
        this.temp.currentCharacter !== null &&
        this.temp.currentCharacter !== "all" &&
        this.temp.currentCharacter !== "selected" &&
        mod.character === "Unknown"
      ) {
  ...
  async addPreset(presetName) {
    ...
    const newPresetPath = path.join(
      this.config.presetPath,
      `${presetName}.json`,
    );
  ...
  async changePreview(modName, previewPath) {
    ...
    const newPreviewPath = path.join(
      this.config.modSourcePath,
      modName,
      previewFileName,
    );
  ...
  async setLanguage(language) {
    //debug
    console.log(
      `iManager current language: ${this.config.language}, set language: ${language}`,
    );
