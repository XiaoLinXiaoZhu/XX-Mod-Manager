...
const _addConfig = {
  ...
  onChange: (value) => {
    ...
    TapeConfig.createConfig(newConfigName.data)
      .then(() => {
...
const _createShortOfCurrentConfig = {
  ...
  onChange: (_value) => {
    ...
    iManager
      .createAppShortCut(changeConfig.data)
      .then(() => {
...
const _ifKeepModNameAsModFolderName = {
  ...
  onChange: (value) => {
    ...
    _ifKeepModNameAsModFolderName.data = value;

    iManager.config.ifKeepModNameAsModFolderName = value;
  },
};

const _ifUseTraditionalApply = {
  ...
  t_description: {
    zh_cn:
      "开启后，mod将以传统方式应用，使用传统模式时，将会通过更改文件夹名称来变更mod应用状态",
  ...
  onChange: (value) => {
    ...
    if (value) {
      if (iManager.config.modTargetPath !== iManager.config.modSourcePath) {
        ...
        console.log(
          "ifUseTraditionalApply changed:",
          value,
          "modTarget and modSource need to be the same folder",
        );
        t_snack(
          {
      ...
      if (iManager.config.ifKeepModNameAsModFolderName) {
        ...
        console.log(
          "ifUseTraditionalApply changed:",
          value,
          "ifKeepModNameAsModFolderName need to be false",
        );
        t_snack(
          {
            zh_cn:
              "使用该功能需要 关闭 保持mod名称与mod文件夹名称一致，请在 设置/常规设置 中关闭",
      ...
      _ifUseTraditionalApply.data = true;
      iManager.config.ifUseTraditionalApply = true;
      // return true;
    } else {
      // debug
      console.log("ifUseTraditionalApply changed:", value);
      _ifUseTraditionalApply.data = false;
      iManager.config.ifUseTraditionalApply = false;
      // return false;
    }
  },
