...
function getData() {
  ...
  //-------------------- 是否使用上次使用的预设 ------------------//
  const ifStartWithLastPresetData = {
  ...
  const modSourcePathData = {
    ...
    t_description: {
      zh_cn:
        "mod来源文件夹为程序存储mod的位置，当mod被启用时，会从这里创建链接到mod目标文件夹。",
  ...
  const presetPathData = {
    ...
    t_description: {
      zh_cn:
        "预设文件夹为存储预设的位置，预设是一组mod的集合，可以通过预设一键启用多个mod",
