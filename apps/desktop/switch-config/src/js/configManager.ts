...
class TapeConfig {
  ...
  public static async getConfigRootPath() {
    ...
    return TapeConfig._configRootPath;
  }
  ...
  constructor(dir: string) {
    ...
    this.tape_front =
      ["jpg", "jpeg", "png", "gif"]
        .map((ext) => path.join(this._dir, `front.${ext}`))
        .find(fs.existsSync) || "";
    this.tape_side =
      ["jpg", "jpeg", "png", "gif"]
        .map((ext) => path.join(this._dir, `side.${ext}`))
        .find(fs.existsSync) || "";
    this.tape_back =
      ["jpg", "jpeg", "png", "gif"]
        .map((ext) => path.join(this._dir, `back.${ext}`))
        .find(fs.existsSync) || "";
    this.tape_spine =
      ["jpg", "jpeg", "png", "gif"]
        .map((ext) => path.join(this._dir, `spine.${ext}`))
        .find(fs.existsSync) || "";
  ...
  static async loadAllConfig() {
    ...
    // 遍历 config
    const configDirs: string[] = fs
      .readdirSync(configRootDir)
      .filter((file: string): boolean =>
        fs.statSync(path.join(configRootDir, file)).isDirectory(),
      );
    // 如果没有配置文件夹，创建一个
  ...
  static async getAllConfig() {
    if (TapeConfig._allConfig.length === 0) {
      TapeConfig._allConfig = await TapeConfig.loadAllConfig();
    }
    return TapeConfig._allConfig;
  }
