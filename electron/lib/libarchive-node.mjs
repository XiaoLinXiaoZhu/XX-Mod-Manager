import { URL as t } from "node:url";
import { Worker as e } from "node:worker_threads";
...
function l(e, t = globalThis, n = ["*"]) {
	t.addEventListener("message", function r(s) {
		...
		try {
			...
			switch (c) {
				case "GET":
					...
					break;
				case "SET":
					(t[p.slice(-1)[0]] = b(s.data.value)), (f = !0);
					break;
				case "APPLY":
					f = n.apply(t, h);
					break;
		...
		Promise.resolve(f)
			...
			.then((n) => {
				const [s, a] = E(n);
				t.postMessage(Object.assign(Object.assign({}, s), { id: o }), a),
					"RELEASE" === c &&
	...
	const a = new Proxy(n, {
		get(_n, r) {
			...
			if ("then" === r) {
				...
				const n = R(e, { type: "GET", path: t.map((e) => e.toString()) }).then(
					b,
				);
		...
		set(_n, r, s) {
			...
			return R(
				e,
				{ type: "SET", path: [...t, r].map((e) => e.toString()), value: a },
				o,
			).then(b);
		},
		apply(_n, _s, a) {
			...
			if (o === r) return R(e, { type: "ENDPOINT" }).then(b);
			if ("bind" === o) return g(e, t.slice(0, -1));
			const [c, l] = w(a);
			return R(
				e,
...
function P(e) {
	if (e instanceof File || e instanceof k || null === e) return e;
	const t = {};
...
function S(e, t = "") {
	...
	for (const r of Object.keys(e))
		e[r] instanceof File || e[r] instanceof k || null === e[r]
...
function _(e, t) {
	...
	let r = e,
		s = null;
	for (const e of n) (r[e] = r[e] || {}), (s = r), (r = r[e]);
...
class A {
	...
	async getFilesObject() {
		...
		return (
			(await this.client.listFiles()).forEach((e) => {
				const [t, n] = _(this._content, e.path);
				"FILE" === e.type &&
					(t[n] = new k(e.fileName, e.size, e.path, e.lastModified, this));
			}),
			(this._processed = 1),
			P(this._content)
		);
	...
	getFilesArray() {
		return this.getFilesObject().then((e) => S(e));
	}
	...
	async extractFiles(e = void 0) {
		...
		return (
			(await this.client.extractFiles()).forEach((t) => {
				const [n, r] = _(this._content, t.path);
				"FILE" === t.type &&
...
class z {
	static init(e = null) {
		return (z._options = e || {}), z._options;
	}
	static async open(e) {
		const t = z.getWorker(z._options),
			n = await z.getClient(t, z._options),
			r = new A(e, n, t);
		return await r.open();
	}
	static async write({
		...
		const i = z.getWorker(z._options),
			a = await z.getClient(i, z._options),
			o = await a.writeArchive(e, n, r, s);
		return (
			i.terminate(), new File([o], t, { type: "application/octet-stream" })
		);
	}
	static getWorker(e) {
		return e.getWorker
			? e.getWorker()
			: new Worker(
					e.workerUrl || new URL("./worker-bundle.js", import.meta.url),
					{ type: "module" },
				);
	}
	static async getClient(e, t) {
		var n;
		const r =
			(null === (n = t.createClient) || void 0 === n ? void 0 : n.call(t, e)) ||
			p(e);
...
const C = new t(".", import.meta.url).pathname;
z.init({
	getWorker: () => new e(`${C}/worker-bundle-node.mjs`),
	createClient: (e) =>
		p(
			/**
			 * @license
			...
			((e) => {
				...
				return {
					...
					addEventListener: (_n, r) => {
						const s = (e) => {
							"handleEvent" in r ? r.handleEvent({ data: e }) : r({ data: e });
						};
						e.on("message", s), t.set(r, s);
					},
					removeEventListener: (_n, r) => {
						const s = t.get(r);
						s && (e.off("message", s), t.delete(r));
					},
...
export { L as ArchiveFormat, O as ArchiveCompression, z as Archive };
