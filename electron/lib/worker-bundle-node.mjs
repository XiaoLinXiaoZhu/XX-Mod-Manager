import { parentPort as e } from "node:worker_threads";
...
function u(e, t = globalThis, n = ["*"]) {
	t.addEventListener("message", function s(i) {
		...
		const { id: c, type: d, path: h } = Object.assign({ path: [] }, i.data),
			f = (i.data.argumentList || []).map(g);
		let m;
		try {
			const t = h.slice(0, -1).reduce((e, r) => e[r], e),
				n = h.reduce((e, r) => e[r], e);
			switch (d) {
				case "GET":
					m = n;
					break;
				case "SET":
					(t[h.slice(-1)[0]] = g(i.data.value)), (m = !0);
					break;
				case "APPLY":
					m = n.apply(t, f);
					break;
				case "CONSTRUCT":
					m = ((e) => Object.assign(e, { [r]: !0 }))(new n(...f));
					break;
				case "ENDPOINT":
					{
						const { port1: r, port2: t } = new MessageChannel();
						u(e, t), (m = ((e, r) => (v.set(e, r), e))(r, [r]));
					}
					break;
				case "RELEASE":
					m = void 0;
		...
		Promise.resolve(m)
			...
			.then((r) => {
				const [n, a] = w(r);
				t.postMessage(Object.assign(Object.assign({}, n), { id: c }), a),
					"RELEASE" === d &&
	...
	const s = new Proxy(o, {
		get(_t, o) {
			...
			if ("then" === o) {
				...
				const t = _(e, { type: "GET", path: r.map((e) => e.toString()) }).then(
					g,
				);
		...
		apply(_n, _o, s) {
			...
			if (i === t) return _(e, { type: "ENDPOINT" }).then(g);
			if ("bind" === i) return m(e, r.slice(0, -1));
			const [u, l] = p(s);
			return _(
				e,
	...
	*entries(e = !1, r = null) {
		...
		for (
			...
			const n = {
				...
				type: y[this._runCode.getEntryType(t)],
...
class b {
	...
	async write(e, r, t, n = null) {
		...
		const u = this._runCode.closeArchiveWrite(i),
			l = this._runCode.freeArchiveWrite(i);
		if (0 !== u || 0 !== l) throw new Error(this._runCode.getError(i));
		const c = this.readNumberFromPointer(s);
	...
	S =
		...
		async (e = {}) => {
			...
			var oe = {
				...
				se = {
					ops_table: null,
					mount: (_e) => se.createNode(null, "/", 16895, 0),
					createNode(e, r, t, n) {
					...
					stream_ops: {
						...
						write(e, r, t, n, o, a) {
							...
							if (
								...
								if (a)
									return (
										(s.contents = r.subarray(t, t + n)), (s.usedBytes = n), n
									);
				...
				le = {
					...
					mount(e, r, t) {
						...
						return (
							...
							o
								? (le.root = u)
								: n && ((n.mounted = i), n.mount?.mounts.push(i)),
							u
						);
					},
					unmount(e) {
				...
				fe = {
					...
					stream_ops: {
						...
						write(e, r, t, n, _o) {
							...
							if (l >= i) return u.buffer.set(s, u.offset), (u.offset += i), i;
							l > 0 &&
...
class C {
	constructor(e) {
		(C.readyCallback = e), A && setTimeout(() => e(), 0);
	}
	open(e, r) {
		D.open(e).then(() => r());
	}
	listFiles() {
		const e = [];
		for (const r of D.entries(!0)) e.push(r);
		return e;
	}
	extractFiles() {
		const e = [];
		for (const r of D.entries(!1)) e.push(r);
		return e;
	}
	extractSingleFile(e) {
		for (const r of D.entries(!0, e)) if (r.fileData) return r;
	}
	hasEncryptedData() {
		return D.hasEncryptedData();
	}
	usePassword(e) {
		D.setPassphrase(e);
	}
...
var P;
(P = (e) => {
	(D = new E(e)), (M = new b(e)), C?.readyCallback?.(), (A = !0);
}),
	S(new F()).then((e) => {
		e.initFunctions(), P(e);
	}),
		...
		((e) => {
			...
			return {
				...
				addEventListener: (_t, n) => {
					const o = (e) => {
						"handleEvent" in n ? n.handleEvent({ data: e }) : n({ data: e });
					};
					e.on("message", o), r.set(n, o);
				},
				removeEventListener: (_t, n) => {
					const o = r.get(n);
					o && (e.off("message", o), r.delete(n));
				},
