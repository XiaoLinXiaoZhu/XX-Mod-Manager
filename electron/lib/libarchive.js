		...
		try {
			...
			switch (u) {
				case "GET":
					...
					break;
				case "SET":
					(t[p.slice(-1)[0]] = E(a.data.value)), (f = !0);
					break;
				case "APPLY":
					f = n.apply(t, h);
					break;
	...
	const a = new Proxy(i, {
		get(_t, i) {
			...
			if ("then" === i) {
				...
				const t = v(e, { type: "GET", path: r.map((e) => e.toString()) }).then(
					E,
				);
		...
		apply(_n, _i, a) {
			...
			if (o === t) return v(e, { type: "ENDPOINT" }).then(E);
...
function R(e) {
	if (e instanceof File || e instanceof b || null === e) return e;
	const t = {};
...
function S(e, t = "") {
	...
	for (const r of Object.keys(e))
		e[r] instanceof File || e[r] instanceof b || null === e[r]
	...
	async getFilesObject() {
		...
		return (
			(await this.client.listFiles()).forEach((e) => {
				const [t, n] = A(this._content, e.path);
				"FILE" === e.type &&
					(t[n] = new b(e.fileName, e.size, e.path, e.lastModified, this));
			}),
			(this._processed = 1),
			R(this._content)
		);
	...
	async extractFiles(e = void 0) {
		...
		return (
			(await this.client.extractFiles()).forEach((t) => {
				const [n, r] = A(this._content, t.path);
				"FILE" === t.type &&
...
class O {
	...
	static async getClient(e, t) {
		...
		const r =
			...
			l(e);
		const { promise: i, resolve: s } = Promise.withResolvers();
		const a = await new r(
			w(() => {
				s();
			}),
...
O._options = {};
export{O as Archive,k as ArchiveCompression,P as ArchiveFormat};
