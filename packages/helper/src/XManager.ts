...
// 导入 Language
import { type Language, setCurrentLanguage, TranslatedText } from "./Language";
// 导入 PathHelper
...
class XManager {
  ...
  async loadConfig() {
    ...
    if (currentConfig == null || Object.keys(currentConfig).length === 0) {
      // snack('配置文件不存在');
      t_snack(
        new TranslatedText("Config file not found", "配置文件不存在"),
        SnackType.error,
      );
      // 保存默认配置
      this.saveConfig();
  ...
  async loadMods() {
    ...
    console.log(
      new TranslatedText(
        `loadMods from ${modSourcePath}`,
        `从 ${modSourcePath} 加载mod`,
      ),
    );
    ...
    if (
      !PathHelper.CheckDir(
        ...
        false,
        true,
        new TranslatedText("Mod Source Path", "Mod 源路径"),
      )
    ) {
      return;
    }
    ...
    if (loadMods.length === 0) {
      ...
      t_snack(
        new TranslatedText("Mod list is empty", "mod列表为空"),
        SnackType.error,
      );
      return;
    }
  ...
  async getModInfo(modName) {
    ...
    //! 改为直接从 data 中获取
    const data = this.data.modList.find(
