...
function runCommand(iManager, command) {
  ...
  if (!command) {
    const snackMessage =
      iManager.config.language === "zh_cn" ? "命令未设置" : "Command not set";
    iManager.snack(snackMessage, "error");
    return false;
  }
...
function autoStart(iManager) {
  ...
  console.log(
    "autoStart:",
    iManager.getPluginData(pluginName, "autoStartModLoader"),
    iManager.getPluginData(pluginName, "autoStartGame"),
  );
  iManager.snack("Trying to auto start mod loader and game...");
  const commandStatus = {
    data: iManager.getPluginData(pluginName, "commandStatus"),
  };
  if (iManager.getPluginData(pluginName, "autoStartModLoader"))
    startModLoader(
      iManager,
      iManager.getPluginData(pluginName, "modLoaderPath"),
    );
  ...
  if (iManager.getPluginData(pluginName, "ifRunCommand")) {
    ipcRenderer.invoke("get-args").then((args) => {
      ...
      console.log(
        ...
        args,
        args.switchConfig,
        iManager.getPluginData(pluginName, "ignoreSwitchConfig"),
      );
      if (
        !args.switchConfig ||
        iManager.getPluginData(pluginName, "ignoreSwitchConfig")
  ...
  if (
    iManager.getPluginData(pluginName, "commandStatus") === ignoredState &&
    !window.location.href.includes("switchConfig")
  ) {
    runCommand(iManager, iManager.getPluginData(pluginName, "command"));
    // 重置 commandStatus
    commandStatus.data = "";
...
module.exports = {
  ...
  init(iManager) {
    ...
    const _ignoredState = "ignoreSwitchConfig,waiting for switchConfig";
    ...
    const autoStartModLoader = {
      ...
      onChange: (value) => {
        // 检查 mod 加载器路径是否存在
        const modLoaderPath = iManager.getPluginData(
          pluginName,
          "modLoaderPath",
        );
        if (value && !modLoaderPath && !fs.existsSync(modLoaderPath)) {
          const snackMessage =
            iManager.config.language === "zh_cn"
              ? "Mod加载器路径未设置或不存在"
              : "Mod Loader Path not set or not exist";
          iManager.snack(snackMessage, "error");
          return false;
        }
      ...
      onChange: (value) => {
        ...
        if (value && !gamePath && !fs.existsSync(gamePath)) {
          const snackMessage =
            iManager.config.language === "zh_cn"
              ? "游戏路径未设置或不存在"
              : "Game Path not set or not exist";
          iManager.snack(snackMessage, "error");
          return false;
        }
