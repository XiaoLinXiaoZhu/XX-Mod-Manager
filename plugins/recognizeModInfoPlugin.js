  ...
  lines.forEach((line) => {
    ...
    if (keyMatch) {
      ...
      for (let i = 0; i < mainKeyIndex; i++) {
        ...
        if (
          !modifiers.includes(mappedModifier) &&
          !excludedModifiers.has(mappedModifier)
        ) {
      ...
      if (iManager?.config && iManager.config.language === "zh_cn") {
...
module.exports = {
  ...
  init(iManager) {
    ...
    pluginData.push({
      ...
      t_description: {
        zh_cn:
          "开启后，导入mod时添加切换键信息,它只用来识别按键，并不能知道按键的具体功能",
    ...
    const ifRefreshAllMod = {
      ...
      onChange: (value) => {
        ...
        if (value) {
          const snackMessage = {
            zh_cn:
              "刷新全部的mod的切换键可能会需要一些时间，这取决于你的mod数量，请坐和放宽，不要关闭程序",
