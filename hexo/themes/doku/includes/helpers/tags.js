module.exports = (hexo) => {
  ...
  const check_navbar_config = (navbar) => {
    if (!navbar?.links || !Object.keys(navbar.links).length) {
      setTimeout(() => {
        hexo.log.error(
          "Invalid navbar configuration. Please check your navbar config in theme/doku/_config.yml",
        );
      }, 100);
      return false;
    }
    for (const [lang, items] of Object.entries(navbar.links)) {
      if (!Array.isArray(items) || !items.length) {
        setTimeout(() => {
          hexo.log.error(
            `Navbar links for language "${lang}" are empty or invalid. Please check your navbar config in theme/doku/_config.yml`,
          );
        }, 100);
        return false;
      }
      for (const { text, url } of items) {
        if (!url) {
          setTimeout(() => {
            hexo.log.error(
              `Invalid navbar item in language "${lang}". Each item must have a valid "url". Please check your navbar config in theme/doku/_config.yml`,
            );
          }, 100);
          return false;
        }
    ...
    if (!Object.hasOwn(navbar.links, hexo.config.language)) {
      setTimeout(() => {
        hexo.log.error(
          `Navbar links for language "${hexo.config.language}" are not defined. Please check your navbar config in theme/doku/_config.yml`,
        );
      }, 100);
      return false;
    }
  ...
  function check_sidebar_config(sidebar) {
    if (
      !sidebar ||
      ![1, 2].includes(sidebar.level) ||
      !Array.isArray(sidebar.items)
    ) {
      setTimeout(() => {
        hexo.log.error(
          "Invalid sidebar configuration. Please check your sidebar config in theme/doku/_config.yml",
        );
      }, 100);
      return false;
    }
  ...
  function check_github_config(_github) {
