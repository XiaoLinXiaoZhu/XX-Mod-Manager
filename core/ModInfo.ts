...
class ModInfo {
	...
	public setMetaDataFromJson(json: JSON) {
		...
		if (
			...
			// debug
			console.log(
				`更改模块名称：${this.modName} -> ${newModFolderName} in folder ${this.location}`,
			);
			const newModFolderPath = path.join(
	...
	public rename(newName: string, callback: (err: any) => void) {
		...
		try {
			...
			console.error(
				`重命名模块失败：${this.location} -> ${newModFolderPath}`,
				err,
			);
			callback(err);
			return false;
