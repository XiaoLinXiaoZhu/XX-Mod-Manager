...
class Preset {
  ...
  static fromJson(json: any) {
    return new Preset(
      ...
      json.description,
      json.modIds,
      json.useExperimentalFeature,
      json.experimentalFeatureConfig,
    );
  }
  ...
  build(allPresets: Preset[]) {
    ...
    this.name = fileName;
    // 检查是否有重复的name
    const sameNamePresets = allPresets.filter((p) => p.name === this.name);
    if (sameNamePresets.length > 1) {
      // 有重复的name，需要加上数字
      let index = 1;
      while (true) {
        const newName = `${this.name}(${index})`;
  ...
  getModIds() {
    ...
    // debug
    console.log(
      `${this.name} is using experimental feature, get mod ids from inherited presets`,
    );
    ...
    const getInhertedPresets = (preset: Preset) => {
      if (
        preset.useExperimentalFeature &&
        preset.experimentalFeatureConfig.inheritPresets !== undefined
      ) {
        preset.experimentalFeatureConfig.inheritPresets.forEach(
          (id: string) => {
            const inheritedPreset = PresetHelper.getPresetById(id);
            if (
              inheritedPreset !== undefined &&
              inheritedPreset.id !== preset.id &&
              !inheritedPresets.includes(inheritedPreset) &&
              this.id !== inheritedPreset.id
            ) {
...
class PresetHelper {
  ...
  static loadPresets(folders: string[]) {
    ...
    PresetHelper.presets.forEach((preset) =>
      preset.build(PresetHelper.presets),
    );
  }
  ...
  public static getPresetByName(name: string) {
    if (name === PresetHelper.defaultPresetName) {
      // 警告一下，返回一个空
      console.warn(
        `PresetHelper.getPresetByName: ${name} is default preset, return empty preset`,
      );
