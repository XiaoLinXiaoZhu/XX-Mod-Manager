...
const loadMods = async () => {
	...
	if (
		iManager.temp.currentPreset &&
		iManager.temp.currentPreset !== "default"
	) {
...
// 定义 setModCardRef 方法
const _setModCardRef = (id) => (el) => {
	modCardRefs.value[id] = el;
};

let wasSearching = false;
// 定义 handleFilterChange 方法
const _handleFilterChange = (character) => {
	iManager.setCurrentCharacter(character);
};

const _handleFilterChangeToAll = () => {
...
const _handleFilterChangeToSelected = () => {
...
const _handleFilterChangeToSearch = (search) => {
...
async function changeFilter(character) {
	...
	if (character === "all") {
		...
		document
			.querySelectorAll(`.mod-item[character="${character}"]`)
			.forEach((item) => {
				item.classList.remove("hidden");
			});
	}
